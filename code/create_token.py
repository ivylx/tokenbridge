...
def create():
  ...
  contract_tx_A = contract.functions.createToken(addressA, 'tokenA', 'ERC20s').build_transaction({
    "from": signer.address,
  ...
  contract_tx_B = contract.functions.createToken(addressB, 'tokenB', 'ERC20s').build_transaction({
    "from": signer.address,
    "nonce": w3.eth.get_transaction_count(signer.address),
  })
